...
仓库提供了一个只依赖 Python 3 标准库的 ZRCP/1 打印接收端，供联调协议和控制台使用。它不会驱动真实设备，只会校验握手、会话、控制租约、使能、运动保活与停止，并将动作按中文摘要打印到终端。控制心跳和链路保活仍会在协议层正常处理，但不会刷屏。
...
也可以直接运行已构建的服务。`--lease-seconds` 可用于延长测试租约，默认 60 秒。终端中 `[RX]` 是网关发来的中文动作摘要，`[TX]` 是打印接收端返回的结果摘要，例如“底座前进 0.10 m/s”“控制已启用”。
